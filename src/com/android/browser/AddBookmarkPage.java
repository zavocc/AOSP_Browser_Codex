...
public class AddBookmarkPage extends Activity
    ...
    private Uri getUriForFolder(long folder) {
        if (folder == mRootFolder) {
            return BrowserCompat.BOOKMARKS_DEFAULT_FOLDER_URI;
        }
        return BrowserCompat.buildBookmarksFolderUri(folder);
    }
    ...
    private long addFolderToCurrent(String name) {
        ...
        Uri uri = getContentResolver().insert(
                BrowserCompat.BOOKMARKS_URI, values);
        if (uri != null) {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        View accountsSpinner = findViewById(R.id.accounts);
        if (accountsSpinner != null) {
            View accountRow = (View) accountsSpinner.getParent();
            if (accountRow != null) {
        ...
        onRootFolderFound(BrowserCompat.getDefaultBookmarksRootId(getContentResolver()));
        getLoaderManager().restartLoader(LOADER_ID_EDIT_INFO, null, mEditInfoLoaderCallbacks);
    }
    ...
    private long getTargetFolderForSave() {
        long folder = mCurrentFolder;
        if (folder == DEFAULT_FOLDER_ID) {
            folder = mRootFolder;
        }
        if (folder == DEFAULT_FOLDER_ID) {
            folder = BrowserCompat.getDefaultBookmarksRootId(getContentResolver());
        }
        return folder;
    }
    ...
    private class SaveBookmarkRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                long parent = getTargetFolderForSave();
                Bookmarks.addBookmark(AddBookmarkPage.this, false, url,
                        title, thumbnail, parent);
    ...
    boolean save() {
        ...
        if (mEditingExisting) {
            ...
            values.put(BrowserContract.Bookmarks.PARENT, getTargetFolderForSave());
            if (!mEditingFolder) {
        ...
        public EditBookmarkInfo loadInBackground() {
            ...
            try {
                ...
                if (checkForDupe && info.id == -1 && !TextUtils.isEmpty(url)) {
                    c = cr.query(BrowserCompat.BOOKMARKS_URI,
                ...
                if (info.id != -1) {
                    c = cr.query(ContentUris.withAppendedId(
                            BrowserCompat.BOOKMARKS_URI, info.id),
                            new String[] {
                    ...
                    c.close();
                    c = cr.query(ContentUris.withAppendedId(
                            BrowserCompat.BOOKMARKS_URI, info.parentId),
                            new String[] {
                            BrowserContract.Bookmarks.TITLE,},
                            null, null, null);
                    if (c.moveToFirst()) {
