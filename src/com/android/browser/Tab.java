...
class Tab implements PictureListener {
    ...
    private final WebViewClient mWebViewClient = new WebViewClient() {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            mCurrentState = new PageState(mContext,
                    WebViewCompatibility.isPrivateBrowsingEnabled(view), url, favicon);
    ...
    private void syncCurrentState(WebView view, String url) {
        ...
        mCurrentState.mIncognito = WebViewCompatibility.isPrivateBrowsingEnabled(view);
    }
    ...
    Tab(WebViewController wvcontroller, WebView w, Bundle state) {
        ...
        mCurrentState = new PageState(mContext, w != null
                ? WebViewCompatibility.isPrivateBrowsingEnabled(w) : false);
        mInPageLoad = false;
        mInForeground = false;
    ...
    public Bundle saveState() {
        ...
        mSavedState.putBoolean(INCOGNITO, WebViewCompatibility.isPrivateBrowsingEnabled(mMainView));
        if (mAppId != null) {
    ...
    protected void capture() {
        ...
        if (WebViewCompatibility.getContentWidth(mMainView) <= 0 || mMainView.getContentHeight() <= 0) {
            return;
        }
        Canvas c = new Canvas(mCapture);
        final int left = mMainView.getScrollX();
        final int top = mMainView.getScrollY() + WebViewCompatibility.getVisibleTitleHeight(mMainView);
        int state = c.save();
