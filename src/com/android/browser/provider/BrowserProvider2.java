...
public class BrowserProvider2 extends SQLiteContentProvider {
    ...
    final class DatabaseHelper extends SQLiteOpenHelper {
        ...
        void enableSync(SQLiteDatabase db) {
            ...
            // Keep the local setting disabled for modern non-sync-adapter builds.
            values.put(Settings.VALUE, 0);
        ...
        public void onOpen(SQLiteDatabase db) {
            ...
            normalizeLocalBookmarkParents(db);
        }

        private void normalizeLocalBookmarkParents(SQLiteDatabase db) {
            ContentValues values = new ContentValues();
            values.put(Bookmarks.PARENT, FIXED_ID_ROOT);
            String where = Bookmarks._ID + "!=? AND "
                    + Bookmarks.ACCOUNT_NAME + " IS NULL AND "
                    + Bookmarks.ACCOUNT_TYPE + " IS NULL AND ("
                    + Bookmarks.PARENT + " IS NULL OR "
                    + Bookmarks.PARENT + "<=0 OR "
                    + Bookmarks.PARENT + " NOT IN (SELECT " + Bookmarks._ID + " FROM "
                    + TABLE_BOOKMARKS + "))";
            db.update(TABLE_BOOKMARKS, values, where,
                    new String[] { Long.toString(FIXED_ID_ROOT) });
        }
        ...
        switch (match) {
            ...
            case COMBINED: {
                if ((match == LEGACY || match == LEGACY_ID)
                        && projection == null) {
                    projection = BrowserCompat.HISTORY_PROJECTION;
                }
    ...
    private String[] getAccountNameAndType(long id) {
        ...
        Uri uri = ContentUris.withAppendedId(BrowserCompat.BOOKMARKS_URI, id);
        Cursor c = query(uri,
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (updatingParent
                    ...
                    Uri insertUri = insertInTransaction(BrowserCompat.BOOKMARKS_URI,
    ...
    int updateHistoryInTransaction(ContentValues values, String selection, String[] selectionArgs) {
        ...
        final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        filterSearchClient(selectionArgs);
        Cursor cursor = query(BrowserCompat.HISTORY_URI,
    ...
    protected boolean syncToNetwork(Uri uri) {
        // Browser sync adapters are intentionally disabled for modern standalone builds.
        return false;
    }
