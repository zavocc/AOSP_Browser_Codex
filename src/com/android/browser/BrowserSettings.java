...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    private Runnable mSetup = new Runnable() {
        ...
        public void run() {
            ...
            int memoryClass = am != null ? am.getMemoryClass() : 16;
            if (memoryClass > 16) {
    ...
    private void syncSetting(WebSettings settings) {
        ...
        LegacyWebSettingsCompat.setLightTouchEnabled(settings, enableLightTouch());
        LegacyWebSettingsCompat.setNavDump(settings, enableNavDump());
        ...
        LegacyWebSettingsCompat.setPluginState(settings, getPluginStateValue());
        ...
        LegacyWebSettingsCompat.setSavePassword(settings, rememberPasswords());
        settings.setSaveFormData(saveFormdata());
        settings.setUseWideViewPort(isWideViewport());
        LegacyWebSettingsCompat.setInvertedRendering(settings, useInvertedRendering());
    ...
    private void syncStaticSettings(WebSettings settings) {
        ...
        // zooming
        LegacyWebSettingsCompat.setEnableSmoothTransition(settings, true);
        // disable content url access
        settings.setAllowContentAccess(false);

        // HTML5 API flags
        LegacyWebSettingsCompat.setAppCacheEnabled(settings, true);
        settings.setDatabaseEnabled(true);
        settings.setDomStorageEnabled(true);

        // HTML5 configuration parametersettings.
        LegacyWebSettingsCompat.setAppCacheMaxSize(settings,
                getWebStorageSizeManager().getAppCacheMaxSize());
        LegacyWebSettingsCompat.setAppCachePath(settings, getAppCachePath());
        LegacyWebSettingsCompat.setDatabasePath(settings,
                mContext.getDir("databases", 0).getPath());
        LegacyWebSettingsCompat.setGeolocationDatabasePath(settings,
                mContext.getDir("geolocation", 0).getPath());
    ...
    public void clearCookies() {
        WebViewCompatibility.removeAllCookies(CookieManager.getInstance());
    }

    public void clearHistory() {
        ContentResolver resolver = mContext.getContentResolver();
        BrowserCompat.clearHistory(resolver);
        BrowserCompat.clearSearches(resolver);
    }
    ...
    // TODO: Cache
    public String getPluginStateValue() {
        return mPrefs.getString(PREF_PLUGIN_STATE, "ON");
    }
