...
public class NfcHandler implements NfcAdapter.CreateNdefMessageCallback {
    ...
    public static void register(Activity activity, Controller controller) {
        ...
        setNdefPushMessageCallback(adapter, handler, activity);
    }
    ...
    private static void setNdefPushMessageCallback(
            NfcAdapter adapter, NfcHandler handler, Activity activity) {
        try {
            Method m = adapter.getClass().getMethod("setNdefPushMessageCallback",
                    NfcAdapter.CreateNdefMessageCallback.class, Activity.class, Activity[].class);
            m.invoke(adapter, handler, activity, new Activity[0]);
        } catch (Throwable ignored) {
            // NFC beaming callback API is unavailable on this platform build.
        }
    }
    ...
    final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == GET_PRIVATE_BROWSING_STATE_MSG) {
                mIsPrivate = WebViewCompatibility.isPrivateBrowsingEnabled(mCurrentTab.getWebView());
